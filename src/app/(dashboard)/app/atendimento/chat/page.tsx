...
import { InternalMessageBubble } from "@/components/whatslidia/components/InternalMessageBubble";
import {
  useChatChannels,
  useCompanyUsers,
  useChannelMessages,
...
export default function ChatInternoPage() {
  ...
  const { data: channelMessages, isLoading: isLoadingChannelMessages } = useChannelMessages({
    channelId: selectedChannel?.id || null,
  });
  const { data: directMessages, isLoading: isLoadingDirectMessages } = useDirectMessages(
  ...
  // Use channel messages or direct messages based on selection
  const messages = selectedChannel ? channelMessages : directMessages;
  const isLoadingMessages = selectedChannel ? isLoadingChannelMessages : isLoadingDirectMessages;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-220px)] min-h-[500px] rounded-xl overflow-hidden border border-slate-800 bg-[#0b141a]">
      {/* Header */}
      <div
        className={cn(
      ...
      >
        {selectedChannel || selectedUser ? (
          ...
          <div>
            ...
            <p className={cn("text-sm", isDarkMode ? "text-[#8696a0]" : "text-gray-500")}>
                ...
                {searchMessages.data.messages.length === 0 ? (
                  ...
                  searchMessages.data.messages.map((msg) => (
                    ...
                    >
                      ...
                      <p className={cn("text-sm", isDarkMode ? "text-[#8696a0]" : "text-gray-600")}>
                        {msg.content}
                      </p>
                      <p
                        className={cn("text-xs mt-1", isDarkMode ? "text-[#8696a0]" : "text-gray-400")}
                      >
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          {/* Tabs */}
          <div className={cn("flex border-b", isDarkMode ? "border-[#2a2a2a]" : "border-gray-200")}>
            <button
              ...
              >
                <div className="flex items-end gap-2">
                  ...
                  <Popover open={showEmojiPicker} onOpenChange={setShowEmojiPicker}>
                    ...
                    <PopoverContent
                      side="top"
                      className={cn("p-2 w-auto", isDarkMode ? "bg-[#202c33] border-[#2a2a2a]" : "bg-white")}
                    >
                      ...
                      >
                        {users
                          ?.filter((u) =>
                            u.name
                              .toLowerCase()
                              .includes(messageInput.split("@").pop()?.toLowerCase() || "")
                          )
                          .slice(0, 5)
                          .map((user) => (
                            <button
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              <div
                className={cn(
                  ...
                  isDarkMode
                    ? "bg-gradient-to-br from-[#00a884]/20 to-[#008f6f]/20"
                    : "bg-gradient-to-br from-gray-100 to-gray-200"
                )}
              >
                <MessageCircle
                  className={cn("w-12 h-12", isDarkMode ? "text-[#00a884]" : "text-gray-400")}
                />
              </div>
              <h3
                className={cn("text-2xl font-semibold mb-2", isDarkMode ? "text-[#e9edef]" : "text-gray-900")}
              >
                Chat Interno
              </h3>
              <p
              ...
              >
                <div className="text-center">
                  <p className={cn("text-2xl font-bold text-[#00a884]")}>{channels?.length || 0}</p>
                  <p className={cn("text-xs", isDarkMode ? "text-[#8696a0]" : "text-gray-500")}>Canais</p>
                </div>
                <div className="w-px bg-gray-700" />
                <div className="text-center">
                  <p className={cn("text-2xl font-bold text-[#00a884]")}>{users?.length || 0}</p>
                  <p className={cn("text-xs", isDarkMode ? "text-[#8696a0]" : "text-gray-500")}>Membros</p>
                </div>
                <div className="w-px bg-gray-700" />
                <div className="text-center">
                  <p className={cn("text-2xl font-bold text-green-500")}>{onlineCount}</p>
                  <p className={cn("text-xs", isDarkMode ? "text-[#8696a0]" : "text-gray-500")}>Online</p>
                </div>
              </div>
            </div>
          )}
